        ...
        const fetchData = async () => {
            try {
                ...
                const statusPriority = {
                    'pending': 1, 'processing': 2, 'delivering': 3, 'completed': 4, 'cancelled': 5
                };
        ...
        <AdminLayout title="Quản Lý Đơn Hàng">
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="relative w-96">
                    ...
                    <input type="text" placeholder="Tìm mã đơn, tên khách..." className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                            {loading ? (
                                <tr>
                                    <td colSpan="7" className="p-12 text-center text-slate-500">Đang tải...</td>
                                </tr>
                            ) : orders.length > 0 ? (
                                orders.map((item) => (
                                    <tr key={item.ma_don_hang} className="hover:bg-slate-50/80 transition-colors">
                                        <td className="p-4 font-mono text-slate-500">#{item.ma_don_hang}</td>
                                        <td className="p-4">
                                            <div className="font-bold text-slate-800">{users[item.ma_nguoi_dung] || "Khách vãng lai"}</div>
                                            <div className="text-xs text-slate-400 font-mono mt-0.5">ID: {item.ma_nguoi_dung}</div>
                                        </td>
                                        <td className="p-4 text-xs text-slate-600">{formatDate(item.ngay_dat)}</td>
                                        <td className="p-4 font-bold text-blue-600 text-right">{formatCurrency(item.tong_tien)}</td>
                                        <td className="p-4 text-xs text-slate-500 max-w-[200px]">
                                            <div className="truncate" title={item.dia_chi_giao_hang}><span className="font-semibold">ĐC:</span> {item.dia_chi_giao_hang}</div>
                                            {item.ghi_chu && (<div className="truncate text-slate-400 italic" title={item.ghi_chu}><span className="font-semibold not-italic">Note:</span> {item.ghi_chu}</div>)}
                                        </td>
                                        <td className="p-4 text-center">{getStatusBadge(item.trang_thai)}</td>
                                        <td className="p-4">
                                ...
                                <tr><td colSpan="7" className="p-12 text-center text-slate-500">Chưa có đơn hàng nào.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden max-h-[90vh] overflow-y-auto ring-1 ring-slate-900/5">
                        ...
                        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-lg text-slate-800">Chi tiết đơn hàng #{selectedOrder.ma_don_hang}</h3>
                                    <div className="text-xs text-slate-500">{formatDate(selectedOrder.ngay_dat)}</div>
                                </div>
                            </div>
                            <button onClick={() => setIsDetailModalOpen(false)} className="size-8 flex items-center justify-center rounded-full text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors">
                        ...
                        <div className="p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                                    ...
                                    <p className="text-sm font-medium text-slate-900">{users[selectedOrder.ma_nguoi_dung] || "Khách vãng lai"}</p>
                                    <p className="text-xs text-slate-500 mt-1">ID: {selectedOrder.ma_nguoi_dung}</p>
                                </div>
                                <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                                    <h4 className="font-bold text-slate-700 mb-3 flex items-center gap-2">
                                        <span className="material-symbols-outlined text-sm">local_shipping</span> Giao hàng
                                    </h4>
                                    <p className="text-sm text-slate-600">{selectedOrder.dia_chi_giao_hang}</p>
                            ...
                            <div className="border rounded-xl overflow-hidden mb-6">
                                <table className="w-full text-left text-sm">
                                    ...
                                    <tbody className="divide-y">
                                        ...
                                        {selectedOrder.chi_tiet_don_hangs && selectedOrder.chi_tiet_don_hangs.length > 0 ? (
                                            selectedOrder.chi_tiet_don_hangs.map((ct, index) => (
                                                <tr key={index}>
                                                    <td className="p-3">
                                                        <div className="font-medium text-slate-800">
                                                            {/* Tên sản phẩm lấy từ relation san_pham */}
                                                            {ct.san_pham?.ten_san_pham || `Sản phẩm #${ct.ma_san_pham}`}
                                                        </div>
                                                    </td>
                                                    <td className="p-3 text-center">{ct.so_luong}</td>
                                                    <td className="p-3 text-right text-slate-600">{formatCurrency(ct.gia_mua)}</td>
