...
export default function AdminDashboard() {
    ...
    // Cấu hình URL API
    const API_URL = "http://127.0.0.1:8000/api/don-hang";
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                const uniqueUsers = new Set(orders.map(order => order.ma_nguoi_dung)).size;
                ...
                const sortedOrders = [...orders].sort((a, b) => 
                    new Date(b.created_at || b.ngay_dat) - new Date(a.created_at || a.ngay_dat)
                );
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm ring-1 ring-slate-200">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="material-symbols-outlined text-blue-500 bg-blue-50 p-2 rounded-lg">receipt_long</span>
                    </div>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm ring-1 ring-slate-200">
                    ...
                    <p className="text-3xl font-bold text-slate-800">{loading ? "..." : formatCurrency(stats.revenue)}</p>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm ring-1 ring-slate-200">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="material-symbols-outlined text-purple-500 bg-purple-50 p-2 rounded-lg">group</span>
                    </div>
                    <p className="text-3xl font-bold text-slate-800">{loading ? "..." : stats.uniqueCustomers}</p>
                        ...
                        <tbody className="divide-y divide-slate-100 text-sm">
                            {loading ? (
                                ...
                                recentOrders.map((order) => (
                                    <tr key={order.ma_don_hang} className="hover:bg-slate-50 transition-colors">
                                        <td className="px-6 py-4 font-bold text-blue-600">#{order.ma_don_hang}</td>
                                        <td className="px-6 py-4 font-medium text-slate-700">
                                            User ID: {order.ma_nguoi_dung}
                                        </td>
                                        <td className="px-6 py-4 text-slate-500">
                                            {new Date(order.created_at || order.ngay_dat).toLocaleDateString('vi-VN')}
                                            <br/>
                                            <span className="text-xs">{new Date(order.created_at || order.ngay_dat).toLocaleTimeString('vi-VN')}</span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            ...
                                            <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold uppercase ${getStatusColor(order.trang_thai)}`}>
