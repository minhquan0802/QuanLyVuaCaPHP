  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && (data.status || data.success)) {
        // Laravel thường trả về 'status' khi reset pass thành công
        setMessage(
      ...
      <div className="w-full max-w-[480px]">
        <div className="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100 p-8 md:p-10">
          ...
          {!message && (
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <button
                type="submit"
                disabled={loading}
                className={`mt-2 w-full py-3.5 rounded-xl bg-blue-600 text-white font-bold shadow-lg hover:bg-blue-700 transition-all ${
          ...
          <div className="mt-8 text-center">
            <Link
              to="/"
              className="text-sm font-bold text-gray-500 hover:text-blue-600 flex items-center justify-center gap-2"
            >
