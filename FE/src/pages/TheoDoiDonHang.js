...
export default function TheoDoiDonHang() {
    ...
    const getStatusColor = (status) => {
        switch (status) {
            ...
            case 'delivering': return 'text-purple-600 border-purple-200 bg-purple-50';
    ...
    const fetchOrders = async () => {
        ...
        try {
            setLoading(true);
            const res = await fetch(`${APP_BASE_URL}/don-hang?ma_nguoi_dung=${userId}`);
    ...
    const handleUpdateStatus = async (orderId, newStatus) => {
        ...
        try {
            ...
            if (res.ok && result.success) {
                alert(newStatus === 'cancelled' ? "Đã hủy đơn hàng thành công" : "Cảm ơn bạn đã mua hàng!");
                fetchOrders();
            } else {
        ...
        <div className="bg-slate-50 font-body text-slate-600 min-h-screen flex flex-col">
            ...
            <main className="flex-grow pt-24 pb-12 px-4 sm:px-6">
                <div className="mx-auto max-w-5xl">
                    <h1 className="font-display text-2xl font-bold text-blue-900 mb-6">Đơn mua</h1>
                            ...
                            {tabs.map((tab) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {tab.id !== 'all' && <span className="material-symbols-outlined text-[18px]">{tab.icon}</span>}
                    ...
                    <div className="space-y-4 mt-4">
                        {filteredOrders.length === 0 ? (
                            ...
                            filteredOrders.map((order) => (
                                <div key={order.ma_don_hang} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                                    
                                    {/* Header Card */}
                                    <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/30">
                                        <div className="text-sm text-slate-500">
                                            Mã đơn: <span className="font-bold text-slate-700">#{order.ma_don_hang}</span>
                                            <span className="mx-2">|</span>
                                            {new Date(order.ngay_dat).toLocaleString('vi-VN')}
                                        </div>
                                        <div className={`px-3 py-1 rounded text-xs font-bold uppercase border ${getStatusColor(order.trang_thai)}`}>
                                        ...
                                        {order.chi_tiet_don_hangs?.map((item, idx) => (
                                            <div key={idx} className="p-4 flex gap-4 hover:bg-slate-50 transition-colors">
                                                <div className="size-20 border border-gray-200 rounded flex-shrink-0 overflow-hidden bg-white">
                                                    <img 
                                                        src={item.san_pham?.hinh_anh || "https://placehold.co/100"} 
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-sm text-gray-500">Số lượng: x{item.so_luong}</p>
                                                </div>
                                    ...
                                    <div className="bg-orange-50/20 p-6 border-t border-gray-100">
                                        ...
                                        <div className="flex justify-end gap-3">
                                            ...
                                            {order.trang_thai === 'pending' && (
                                                <button 
                                                    onClick={() => handleUpdateStatus(order.ma_don_hang, 'cancelled')}
                                            ...
                                            {order.trang_thai === 'delivering' && (
                                                <button 
                                                    onClick={() => handleUpdateStatus(order.ma_don_hang, 'completed')}
